...
if __name__ == "__main__":
	...
	'''
	db = './oxford_flow/train'
	db_test = './oxford_flow/test'
	db_valid = './oxford_flow/valid'
	classes = len(os.listdir(db))
	base_model = applications.VGG16(weights='imagenet',input_shape = (input_shape[0], input_shape[1], input_shape[2]),include_top=False)
	x = base_model.output
	x = Flatten(input_shape=base_model.output_shape[1:],name='flatten')(x)
	x = Dense(256, activation='relu',name='F1')(x)
	x = Dropout(0.2)(x)
	...
	if aug == '1':#augmentation
		...
		if not args['test']:
			# fine-tune the model
			checkpointer = ModelCheckpoint(filepath='./models/finetune_wsx.hdf5',verbose=2,save_best_only=True)
			earlystopper = EarlyStopping(monitor='val_loss', patience=10, verbose=2)
			model.fit_generator(
					train_generator,
		...
		model.load_weights("./models/finetune_wsx.hdf5")
